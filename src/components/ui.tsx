...
export function Textarea(props: React.TextareaHTMLAttributes<HTMLTextAreaElement>) {
  return (
    <textarea
      rows={3}
      {...props}
      className={`w-full resize-y rounded-lg border border-slate-300 px-3 py-2 text-sm text-navy-950 outline-none focus:border-navy-600 focus:ring-2 focus:ring-navy-100 ${props.className ?? ''}`}
    />
  );
}
